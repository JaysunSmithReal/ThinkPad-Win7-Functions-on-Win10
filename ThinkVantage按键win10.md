...
通常在开机状态下按下该按键可打开机体设置或 IBM 技术支持程序
...
## 以下是在Win0环境下重新映射ThinkVantage 按键的方法：

操作前请确认已安装`Hotkey Features Integration（热键驱动程序 ）`,确认方法是当麦克风静音时指示灯是点亮状态

一、以管理员权限打开注册表编辑器

二、定位到 `HKEY_LOCAL_MACHINE\SOFTWARE\Lenovo\ShortcutKey\AppLaunch`

三、右键菜单>新建>项，命名为：`Ex_17`

四、选中 Ex_17，在右侧新建 DWORD 值，命名为 `AppType` 并赋值 `10001（十六进制）` #Win10 21H2+等高版本系统，请将EX_17下的AppType键值设置为1

五、在 Ex_17 下右键菜单>新建>项，命名为：`Desktop`

六、在 Desktop 下转向右侧，创建字符串 `File` 和 `Parameters`，File 中填写要启动的应用/要打开的文件所在路径，Parameters 留空
 
重启电脑，按下 ThinkVantage 蓝色按键即可启动在上一步指定的应用或文件
